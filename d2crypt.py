...
class Decrypter():
    ...
    def determine_packet_size(self, packet):
        if packet[0] < 0xf0:
            return (1, packet[0] - 1)
        else:
    ...
    def decrypt(self, packet):
        packets = self.head + packet
        ret = ()
        while len(packets) > 1:
            shift, size = self.determine_packet_size(packets)
            if shift + size > len(packets):
                break

            fpacket, packets = packets[:shift + size], packets[shift + size:]

            pack, count = bytearray(), 0x20
            a = b = c = d = index = 0
            while True:
                while size and count >= 8:
                    size -= 1
                    count -= 8
                    b |= fpacket[shift] << count
                    shift += 1
                ...
                pack.append(character_table[index + 2 * d + 1])
                b <<= (c & 0xFF)
                b &= 0xffffffff
        self.head = packets
        return ret
...
def create_game_packet_size_header(packet):
    size = len(packet)
    if size > 238:
        size = (size + 2) | 0xF000
...
def encrypt(packet):
    ...
    while size != 0:
        a = compression_table[packet[i]]
        e = (a >> 8) & 0xff
        buf = buf | ((a >> 24) << (24 - count))
        count = count + ((a >> 16) & 0xff)
        if e != 0:
            buf = buf | (((a & 0xFF) << (8 - e)) << (24 - count))
            count = count + e
        while count > 8:
            ret.append(buf >> 24)
            count = count - 8
    ...
    return create_game_packet_size_header(ret) + bytes(ret)
